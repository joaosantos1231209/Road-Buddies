...
// Extend the insertTripSchema from shared/schema.ts for updating
...
type UpdateTripFormValues = z.infer<typeof updateTripSchema>;

export default function EditTripModal({ isOpen, onClose, trip }: EditTripModalProps) {
  const { toast } = useToast();
  const { currentUser } = useAuth();
  const [defaultSeats, setDefaultSeats] = useState<number>(trip.availableSeats || 4); // Default from trip or fallback to 4
  ...
  const form = useForm<UpdateTripFormValues>({
    ...
    defaultValues: {
      ...
      originRadius: trip.originRadius,
      destinationName: trip.destinationName,
      destinationLat: trip.destinationLat,
      destinationLng: trip.destinationLng,
      destinationRadius: trip.destinationRadius,
      startDate: new Date(trip.startDate),
      endDate: trip.endDate ? new Date(trip.endDate) : undefined,
      departureTime: trip.departureTime || "",
      // Removemos "FLEXIBLE" do tipo para corresponder ao que foi definido no schema
      status: trip.status as "PROVIDER" | /* "FLEXIBLE" | */ "NEEDRIDE",
      availableSeats: trip.availableSeats || 1,
      notes: trip.notes || "",
    },
  });
  
  // Fetch user profile data to get default seats
  useEffect(() => {
    const fetchUserData = async () => {
      if (currentUser?.uid) {
        try {
          const userData = await apiRequest(`/api/users/by-firebase-id/${currentUser.uid}`);
          if (userData && userData.vehicleSeats) {
            setDefaultSeats(userData.vehicleSeats);
          }
        } catch (error) {
          console.error('Error fetching user data for seat defaults:', error);
        }
      }
    };
  ...
  function onSubmit(data: UpdateTripFormValues) {
    // Make sure dates are properly formatted for Drizzle/PostgreSQL
    const startDate = dateRange?.from ? new Date(dateRange.from) : 
                     data.startDate ? new Date(data.startDate) : 
                     new Date(trip.startDate);
    
    const endDate = dateRange?.to ? new Date(dateRange.to) : 
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Editar Viagem</DialogTitle>
          <DialogDescription>
            Atualize os detalhes da sua viagem. Apenas viagens sem participantes podem ser editadas.
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <DialogFooter className="flex justify-end space-x-2">
              ...
              >
                {updateTripMutation.isPending ? "A guardar..." : "Guardar Alterações"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
