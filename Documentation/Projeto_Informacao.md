...
- **Minhas Viagens**: Organizada por viagens ativas, histórico e "Matches", onde o utilizador pode visualizar correspondências para os seus pedidos e utilizar um botão de ação rápida para "Reservar Lugar" diretamente na viagem.
...
- **Autenticação**: Login com Google ou Email/Palavra-Passe, suportado por um sistema de verificação OTP (One-Time Password), código de 6 dígitos, que permite validar a conta diretamente na plataforma.
...
- **Progressive Web App (PWA)**: A plataforma pode ser instalada diretamente no ecrã principal dos telemóveis, oferecendo uma experiência de aplicação nativa.
- **Notificações Push**: Integração com o telemóvel para receber alertas em tempo real sobre novos Matches, mensagens de Chat, Cancelamento de Viagens e Novos Passageiros.
...
- **Notificações:** Firebase Cloud Messaging (FCM) para o envio de Push Notifications Web/Mobile.
...
4. O condutor pode acompanhar na aba "Minhas Viagens" quem se juntou à viagem e os lugares disponíveis. Pode usar o sistema de mensagens restrito para combinar detalhes com os passageiros. Este chat só é ativado se existirem lugares reservados e o acesso é estritamente limitado ao condutor e aos passageiros confirmados.
...
- **Ocultação Automática**: Assim que um utilizador aceita um match e reserva o seu lugar com sucesso, o seu pedido de boleia (NEEDRIDE) original assume automaticamente o estado de resolvido/oculto, desaparecendo dos pedidos ativos na dashboard para manter a lista limpa.
- **Notificações**: O sistema envia e-mails (via SendGrid) em momentos-chave: quando um match é encontrado, para informar que um utilizador se juntou à viagem, em caso de cancelamento e para solicitações de viaturas aos Serviços Partilhados.
